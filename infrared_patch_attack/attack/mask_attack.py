...
from yolov5.detect import detect_train
...
def shaped_mask_attack_1(H, W, bbox, model, img, device, emp_iterations, max_pertubation_mask = 100, content = 0, grad_avg=False, lambda_sparse=5, lambda_attack=20, lambda_agg=5):
    ## 图片预处理 ##
    X_ori =  torch.stack([trans(img)]).to(device)  # .repeat((3, 1, 1))
    X_ori = F.interpolate(X_ori, (inputsize[0], inputsize[1]), mode='bilinear', align_corners=False) # 采用双线性插值将不同大小图片上/下采样到统一大小

    # X_ori = torch.stack([img]).to(device)  # .repeat((3, 1, 1))
    # X_ori = F.interpolate(X_ori[0], (inputsize[0], inputsize[1]), mode='bilinear',
...
def shaped_mask_attack_2(H, W, bbox, model, img, device, emp_iterations, max_pertubation_mask = 100, content = 0, grad_avg=False, lambda_sparse=5, lambda_attack=20, lambda_agg=5):
    ## 图片预处理 ##
    # X_ori =  torch.stack([trans(img)]).to(device)  # .repeat((3, 1, 1))
    # X_ori = F.interpolate(X_ori, (inputsize[0], inputsize[1]), mode='bilinear', align_corners=False) # 采用双线性插值将不同大小图片上/下采样到统一大小
