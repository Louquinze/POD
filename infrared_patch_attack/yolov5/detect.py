...
def load_model(path="yolov5/weights/best.pt"):
    weights = path
    model = DetectMultiBackend(weights, device=device, dnn=False)
    return model

def load_model_infrared(path="/workspace/shaped_mask_cross_attack/weights/infrared_model.pt"):
    weights = path
    # weights = "/workspace/shaped_mask_cross_attack/yolov5/runs/train/exp10/weights/best.pt"
    model = DetectMultiBackend(weights, device=device, dnn=False)
    return model
...
def detect_train(model,img):
    ...
    loss = torch.sum(pred[0][rank[:end]],dim=0)[4]
...
def detect(model,img,H=120,W=120):
    ...
    return (up, below, left, right), pred[0][0][4].clone().detach(),len(pred[0])-1
