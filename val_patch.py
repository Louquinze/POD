...
def run(
    ...
    # Todo adapt directory
    # cp directory
    # Define the source and destination directories
    n = weights[0].split("/")[2]
    base = list(os.listdir(f'{os.getcwd()}/test/{n}_{n[-1]}'))[0]
    source_dir = f'{os.getcwd()}/test/{n}_{n[-1]}/{base}/proper_patched/images'
        ...
        if weights:
            ...
            with open(os.path.join(save_dir, "metrics.txt"), 'w') as fw:
                if target_class is not None:
                    fw.write(f"map50 {name} {mname} {cname}: {round(map50, 4)}, {round(map50, 4)}\n")
                else:
