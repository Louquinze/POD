...
class PatchTester:
    ...
    def test(self,
        ...
        for imgfile in tqdm.tqdm(img_paths):
            ...
            if save_image:
                original_img = Image.open(imgfile).convert('RGB')
                p_img_pil = transforms_topil(p_tensor_batch.squeeze(0).cpu())
                w, h = p_img_pil.size
                target_size = original_img.size

                # Calculate the bounding box to crop
                left = (w - target_size[0]) / 2
                top = (h - target_size[1]) / 2
                right = (w + target_size[0]) / 2
                bottom = (h + target_size[1]) / 2

                # Crop the image
                p_img_pil = p_img_pil.crop((left, top, right, bottom))
