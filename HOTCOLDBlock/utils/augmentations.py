...
class RandomCutout(object):
    ...
    def __call__(self, img):
        ...
        y1 = np.clip(y - self.size[0] // 2, 0, h)
        y2 = np.clip(y + self.size[0] // 2, 0, h)
        x1 = np.clip(x - self.size[1] // 2, 0, w)
        x2 = np.clip(x + self.size[1] // 2, 0, w)

        img[:, y1:y2, x1:x2] = 0
        return img, (x1+x2) / (2*w), (y1+y2) / (2*h), abs(x2-x1) / w, abs(y2-y1) / h
...
class RandomNoise(object):
    ...
    def __call__(self, img):
        ...
        img[:, y1:y2, x1:x2] = F.normalize(torch.randn_like(img[:, y1:y2, x1:x2]) / 10 + img[:, y1:y2, x1:x2], p=2, dim=0)
        return img, (x1+x2) / (2*w), (y1+y2) / (2*h), abs(x2-x1) / w, abs(y2-y1) / h
