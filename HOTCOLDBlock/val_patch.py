...
def imshow(img, path):
    ...
    # Example tensor of shape (3, 640, 512)
    tensor = (img * 255).detach().cpu().to(torch.float32)
    # Ensure the tensor values are in the range [0, 255]
    tensor = torch.clamp(tensor, min=0, max=255)
    # Convert tensor to uint8
    tensor = tensor.to(torch.uint8)
    # Convert from (3, 640, 512) to (640, 512, 3)
...
def run(
    ...
    target_class = 0
    t = time.time()
    os.makedirs(save_dir / f"eval_{t}")
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        patch_tf, patch_mask_tf = ptp(patch.gbest_position, targets, im)
        # imgWithPatch = im
        im = pa(im, patch_tf, patch_mask_tf)
        ...
        for im_i, path_i in zip(im, paths):
            imshow(im_i, save_dir / f"eval_{t}" / path_i.split("/")[-1])  # Show the first image in the batch
        ...
        if weights:
            ...
            with open(os.path.join(save_dir, "metrics.txt"), 'w') as fw:
                if target_class is not None:
                    fw.write(f"map50 {name} {mname} {cname}: {round(map50, 4)}, {round(map, 4)}\n")
                else:
    ...
    with open(f"{save_dir}/res_hcb_2.txt", "a") as f:
        f.write(f'ASR-------> {1-r}\n'
                f'mAP-------> {map50}, {map}\n'
                f'Precsision: {p}\n\n')
    print('ASR------->', 1-r)
    print(f'mAP-------> {map50}, {map}\n')
    print(f'Precsision: {p}\n')
