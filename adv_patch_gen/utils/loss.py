...
class GeneralProbExtractor(nn.Module):
    """GeneralProbExtractor: extracts objectness probability from YOLO output.

    Module providing the functionality necessary to extract the objectness probability from YOLO output, 
    regardless of the class.

    """

    def __init__(self):
        super(GeneralProbExtractor, self).__init__()
    ...
    def forward(self, output: torch.Tensor):
        ...
        assert output.ndim == 3 and output.size(-1) > 5, "Output tensor shape is invalid."

        # Extract the objectness score from the output tensor
        objectness_score = output[:, :, 4]  # Extracting the objectness score (confidence of any object)

        # Optionally, you could apply a sigmoid here to convert the score to a probability
        objectness_prob = torch.sigmoid(objectness_score)

        # Return the maximum objectness probability across the batch
        max_prob, _ = torch.max(objectness_prob, dim=1)
        return max_prob
