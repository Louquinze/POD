...
class LoadRandomAdvExp(object):
    ...
    def __call__(self, img, labels):
        ...
        l_p = random.randint(4, 32) * 2
        patch = F.interpolate(patch.unsqueeze(0), size=(l_p, l_p),
                              mode='bilinear', align_corners=False)[0]
        patch = np.stack((patch[0],) * 3, axis=-1)

        img_copy = img.copy()
        del img

        rotation_angle = np.random.uniform(0, 360)
        diagonal = int(np.ceil(np.sqrt(2) * l_p))
        padding = (diagonal - l_p) // 2

        padded_patch = np.zeros((diagonal, diagonal, 3), dtype=np.uint8)
        center = diagonal // 2
        padded_patch[center - l_p // 2:center + l_p // 2, center - l_p // 2:center + l_p // 2, :] = patch

        # Rotate the colorful padded patch
        M = cv2.getRotationMatrix2D((center, center), rotation_angle, 1)
        rotated_patch = cv2.warpAffine(padded_patch, M, (diagonal, diagonal))
        shape_correction = (rotated_patch.shape[0] - l_p, rotated_patch.shape[1] - l_p)

        y = np.random.randint(h - l_p - diagonal)
        x = np.random.randint(w - l_p - diagonal)
        img_copy[y:y + l_p + shape_correction[0], x:x + l_p + shape_correction[1], :] \
            = np.where(rotated_patch < 1e-11,
                       img_copy[y:y + l_p + shape_correction[0], x:x + l_p + shape_correction[1], :], rotated_patch)
        return img_copy, (x + round((l_p + shape_correction[0]) / 2)) / w, (y + round(
            (l_p + shape_correction[0]) / 2)) / h, (l_p + diagonal // 4) / w, (l_p + diagonal // 4) / h
    ...
    def __call__(self, img, labels):
        ...
        for lab in labels[labels[:, 0] == 0]:
            if np.random.random() < 0.5:
                ...
                patch_i = \
                    F.interpolate(patch.unsqueeze(0), size=(y2 - y1, x2 - x1), mode='bilinear', align_corners=False)[0]

                img_copy = img.copy()
                del img
                img = img_copy
                patch_i = patch_i.transpose(0, 1).transpose(1, 2)
                img[y1:y2, x1:x2, :] = patch_i
...
class RandomCutout(object):
    ...
    def __call__(self, img, labels):
        h, w = img.shape[0], img.shape[1]
        l_p = random.randint(5, 32) * 2

        img_copy = img.copy()
        del img

        patch = torch.zeros(l_p, l_p, 3) + 0.1
        rotation_angle = np.random.uniform(0, 360)
        diagonal = int(np.ceil(np.sqrt(2) * l_p))
        padding = (diagonal - l_p) // 2
        ...
        padded_patch = np.zeros((diagonal, diagonal, 3), dtype=np.uint8)
        center = diagonal // 2
        padded_patch[center - l_p // 2:center + l_p // 2, center - l_p // 2:center + l_p // 2, :] = 255
        ...
        x = np.random.randint(w - l_p - diagonal)
        img_copy[y:y + l_p + shape_correction[0], x:x + l_p + shape_correction[1], :] \
            = np.where(rotated_patch < 255,
                       img_copy[y:y + l_p + shape_correction[0], x:x + l_p + shape_correction[1], :], 0)
        return img_copy, (x + round((l_p + shape_correction[0]) / 2)) / w, (y + round(
            (l_p + shape_correction[0]) / 2)) / h, (l_p + diagonal // 4) / w, (l_p + diagonal // 4) / h


class RandomGeometry(object):
    def __init__(self, size=640, seed=0, path=""):
        assert isinstance(size, (int, tuple))
        if isinstance(size, int):
            self.size = (size, size)
    ...
    def __call__(self, img, labels):
        ...
        patch = 255 * np.stack((create_random_shapes(l_p, np.random.randint(5, 10)),) * 3, axis=-1)
        rotation_angle = np.random.uniform(0, 360)
        diagonal = int(np.ceil(np.sqrt(2) * l_p))
        padding = (diagonal - l_p) // 2
...
def create_random_shapes(size, n_shapes):
    """
    Create an image with random shapes (circles, ellipses, triangles).
    """
    ...
    for _ in range(n_shapes):
        shape_type = np.random.choice(["circle", "ellipse", "triangle"])
        ...
        if shape_type == "circle":
            ...
            rx, ry = np.random.randint(1, size // 5, 2)
            for i in range(size):
                for j in range(size):
            ...
            for i in range(size):
                for j in range(size):
                    if i < x + radius and j < y + radius and i - j < x - y:
                        image[i, j] = color
    return image + np.random.rand() / 16 * np.random.rand(size, size)
...
class RandomCutoutWhite(object):
    ...
    def __call__(self, img, labels):
        ...
        y1 = np.clip(y - l_p // 2, 0, h)
        y2 = np.clip(y + l_p // 2, 0, h)
        x1 = np.clip(x - l_p // 2, 0, w)
        x2 = np.clip(x + l_p // 2, 0, w)
...
class RandomNoise(object):
    ...
    def __call__(self, img, labels):
        ...
        patch = (np.random.rand(*shape) * 255 * contrast + brightness).clip(0, 255)
        rotation_angle = np.random.uniform(0, 360)
        diagonal = int(np.ceil(np.sqrt(2) * l_p))
        padding = (diagonal - l_p) // 2
...
class RandomNoiseBlur(object):
    ...
    def __call__(self, img, labels):
        ...
        padded_patch = np.zeros((diagonal, diagonal, 1), dtype=np.uint8)
        center = diagonal // 2
        padded_patch[center - l_p // 2:center + l_p // 2, center - l_p // 2:center + l_p // 2, :] = patch

        # Rotate the colorful padded patch
        M = cv2.getRotationMatrix2D((center, center), rotation_angle, 1)
        rotated_patch = np.stack((cv2.warpAffine(padded_patch, M, (diagonal, diagonal)),) * 3, axis=-1)
        shape_correction = (rotated_patch.shape[0] - l_p, rotated_patch.shape[1] - l_p)
        ...
        img_copy[y:y + l_p + shape_correction[0], x:x + l_p + shape_correction[1], :] \
            = np.where(rotated_patch < 1e-11,
                       img_copy[y:y + l_p + shape_correction[0], x:x + l_p + shape_correction[1], :],
                       blur_image(rotated_patch, sigma))
        return img_copy, (x + round((l_p + shape_correction[0]) / 2)) / w, (y + round(
            (l_p + shape_correction[0]) / 2)) / h, (l_p + diagonal // 4) / w, (l_p + diagonal // 4) / h
...
class RandomInv(object):
    ...
    def __call__(self, img, labels):
        ...
        patch = 1 - img_copy[y:y + l_p, x:x + l_p, :]
